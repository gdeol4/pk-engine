...
def read_object_property_file(path, delimiter=',', cols_to_read=[0, 1],
    ...
    return data
# ---------------------------------------------------------------------------
# Reporting helpers used by ReLeaSE
# ---------------------------------------------------------------------------

def print_dict(dict_to_print: dict) -> None:
    """
    Pretty‑print a dictionary whose keys are property names and whose
    values are lists of per‑molecule numbers.
    """
    n_rows = max(len(v) for v in dict_to_print.values())
    header = "Molecule  " + "".join(f"{k[:15]:>20s}" for k in dict_to_print)
    print(header)
    print("-" * len(header))
    for i in range(n_rows):
        row = f"{i+1:8d}  "
        for prop in dict_to_print:
            row += f"{dict_to_print[prop][i]:>20.4f}"
        print(row)
    print()


def print_stats(results: dict, pad: int = 30) -> None:
    """
    Print max / mean / min / std‑dev for every property in *results*.
    """
    for label, fn in {
        "MAXIMUM": np.max,
        "AVERAGES": np.average,
        ...
        row = f"{'':>6s}  {label:{pad}.{pad}s}   "
        for prop in results:
            width = max(len(prop), 6)
            row += f"  {fn(results[prop]):>{width}.2f}"
        print(row)
...
def print_results(
    ...
    """
    Print a table of per‑molecule results and optional summary stats.

    *smiles*   – list of SMILES strings in the same order as *results* lists  
    *results*  – dict {property: list_of_values}  
    """
    if header:
        print(header)
    hdr = f"{'#':>6s}  {'Molecule':{length_lim+3}s}"
    for prop in results:
        hdr += f"  {prop:>{max(len(prop),6)}s}"
    print(hdr)

    for idx, smi in enumerate(smiles):
        cont = "..." if len(smi) > length_lim else "   "
        row = f"{idx:6d}  {smi:{length_lim}.{length_lim}s}{cont}"
        for prop in results:
            row += f"  {results[prop][idx]:>{max(len(prop),6)}.2f}"
        print(row)
...
save_smi_file = save_smi_to_file
